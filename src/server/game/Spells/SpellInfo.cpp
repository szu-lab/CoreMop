...
int32 SpellEffectInfo::CalcValue(Unit const* p_Caster, int32 const* bp, Unit const* target, Item const* itemTarget, int32 itemLevel /*= -1*/) const
{
    ...
    if (ScalingMultiplier != 0.0f)
    {
        if (p_Caster && !_spellInfo->IsCustomCalculated())
        {
            ...
            else // Hack fix for 2hand weapon pvp power scaling with ilvl
            {
                ...
                {
                    if (auto weapon = p_Caster->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, position))
                    {
    ...
    else
    {
        if (p_Caster)
        {
            int32 level = int32(p_Caster->getLevel());
    ...
    float l_Value = float(basePoints);
    ...
    if (BasePoints != basePoints && _spellInfo->Id < 121820)
    {
        ...
        if (sbf)
        {
            for (uint8 i = 0; i < _spellInfo->EffectCount; ++i)
            {
                    ...
                    _spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_MOD_RANGED_ATTACK_POWER))
                {
                    if (_spellInfo->Effects[i].BasePoints > 0)
                        l_Value = float(_spellInfo->Effects[i].BasePoints);
                }
            }
        }
    }
    ...
    l_Value = _spellInfo->GetBonusValueForOldBonus(l_Value);

    if (itemTarget && p_Caster && p_Caster->IsPlayer())
    {
        Player const* p_Player = p_Caster->ToPlayer();
        if (p_Player->GetMap() && p_Player->GetMap()->IsBattlegroundOrArena())
        {
            ...
            if (proto && proto->InventoryType == INVTYPE_TRINKET)
            {
                ...
                if (reduction)
                    l_Value -= CalculatePct(l_Value, reduction);
            }
        }
    }
    ...
    if (p_Caster)
    {
        ...
        if (p_Caster->m_movedPlayer && comboDamage)
            if (uint8 comboPoints = p_Caster->m_movedPlayer->GetComboPoints())
                l_Value += comboDamage * comboPoints;
        ...
        if (!_spellInfo->GetSpellScaling() && !basePointsPerLevel && (_spellInfo->Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION && _spellInfo->SpellLevel) &&
            ...
            //l_Value = int32(l_Value * (int32)getLevel() / (int32)(_spellInfo->spellLevel ? _spellInfo->spellLevel : 1));

        // Hack Fix Arcane Barrage triggered
        if (_spellInfo->Id == 50273)
            l_Value = float(basePoints);
    }
    
    if (_spellInfo->Id == 116010)
        return 25;
...
SpellEffectInfo::StaticData  SpellEffectInfo::_data[TOTAL_SPELL_EFFECTS] =
{
    ...
    { EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT },               ///< 210 SPELL_EFFECT_210
...
bool SpellInfo::IsTargetingEnemySingle() const
{
    for (uint8 i = 0; i < EffectCount; ++i)
        if (Effects[i].TargetA.GetTarget() == TARGET_UNIT_TARGET_ENEMY)
            return true;
    return false;
}
...
{
    return IsPassive() && !HasEffect(SPELL_EFFECT_APPLY_AURA) && !HasEffect(SPELL_EFFECT_APPLY_AURA_2) && !HasEffect(SPELL_EFFECT_APPLY_AREA_AURA_RAID);
}
...
bool SpellInfo::CanCritDamageClassNone() const
{
    switch (Id)
    {
        ...
        case 48153:  // Guarding Spirit (heal spell)
            return true;
    }
    return false;
}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player* player) const
{
    ...
    // bg\arena limitation
    if (AttributesEx8 & SPELL_ATTR8_NOT_IN_BG_OR_ARENA)
    {
        MapEntry const* mapEntry = sMapStore.LookupEntry(map_id);
    ...
    switch (Id)
    {
        case 133676: // Strafing Run, Assault on Zan'vess
            return (map_id == 1050) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 88871: // Summon Siege Tank Turret
        case 88890: // Fire Cannon
            return area_id == 5602 ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 70877:
        case 70946:
            return area_id == 4012 ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 141222: // Call the Shot, Blood on the Snow
            return (map_id == 1130) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 142861: // Ancient Miasma, Malkorok, Siege of Orgrimmar
        case 142906: // Ancient Miasma, Malkorok, Siege of Orgrimmar
        case 142862: // Ancient Barrier, Malkorok, Siege of Orgrimmar
        case 142863: // Weak Ancient Barrier, Malkorok, Siege of Orgrimmar
        case 142864: // Middle Ancient Barrier, Malkorok, Siege of Orgrimmar
        case 142865: // Strong Ancient Barrier, Malkorok, Siege of Orgrimmar
        case 144843: // Overcome, Sha of Pride, Siege of Orgrimmar
        case 144863: // Overcome, Sha of Pride, Siege of Orgrimmar
        case 143840: // Mark of Anguish, Fallen Protectors, Siege of Orgrimmar
        case 144421: // Corruption, Norushen, Siege of Orgrimmar
        case 144452: // Purified, Norushen, Siege of Orgrimmar
            return (map_id == 1136) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 134030 : // Kick Shell
            return (map_id == 1098 && area_id == 6706) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 122220: // Impaling Spear
            return (map_id == 1009) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 32314:
            return (map_id == 530 && (area_id == 3617 || area_id == 3616)) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 123370: // Reshape Life, AMber-Shaper Un'sok, Heart of Fear
        case 122740: // Convert
            return (map_id == 1009) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 66118: // Leeching Swarm, Anub'arak, Trial of Crusader
            return (map_id == 649 && area_id == 4722) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (HasCustomTargetFlag(SpellTargetFlagCu::SPELL_TARGET_FLAG_CU0_NPC_ONLY) && target->GetTypeId() != TYPEID_UNIT)
        return SPELL_FAILED_BAD_TARGETS;
    ...
    if (unitTarget && Id == 50286 && !unitTarget->HasAura(8921) && !unitTarget->HasAura(93402) && caster->HasAura(146655))
     ...
     // Custom MoP Script - Hack fix for Halo (Holy), Halo (Shadow), Piercing Howl, Multi-Shot, Psychic Terror, Earthgrab Totem, Starfall, Barrage - it doesn't break stealth.
     if ((Id == 120517 || Id == 120644 || Id == 12323 || Id == 2643 || Id == 113792 || Id == 3600 || Id == 64695 || Id == 50286 || Id == 120360 || Id == 120361)
         && (unitTarget->HasAuraType(SPELL_AURA_MOD_STEALTH) || HasAura(SPELL_AURA_MOD_INVISIBILITY)) && unitTarget)
         return SPELL_FAILED_BAD_TARGETS;
     ...
     if (unitTarget && unitTarget->ToCreature())
     {
         ...
         {
             case 59390: // Mocking Banner
             case 59398: // Demoralizinbg Banner
             case 59399: // Skull Banner
             {
                 if (Id != 114029 && Id != 46947 && Id != 3411 && Id != 34784)
                     return SPELL_FAILED_BAD_TARGETS;
                 break;
             }
         }
         ...
         {
             if (CreatureTemplate const* proto = unitTarget->ToCreature()->GetCreatureTemplate())
                 if (proto->flags_extra & CREATURE_FLAG_EXTRA_DUMMY)
    ...
    if (!(Targets & TARGET_UNIT_MASTER) && !caster->IsVehicle())
    {
        if (TargetAuraState && !unitTarget->HasAuraState(AuraStateType(TargetAuraState), this, caster))
            // Hack for Shadowburn with Havoc
            if (!(Id == 17877 && implicit))
...
SpellCastResult SpellInfo::CheckExplicitTarget(Unit const* caster, WorldObject const* target, Item const* itemTarget, volatile int32 spellId) const
{
...
AuraStateType SpellInfo::GetAuraState() const
{
    ...
    // Sting (hunter's pet ability) or Flare
    if (Category == 1133 || Id == 94528)
        return AURA_STATE_FAERIE_FIRE;
    ...
    switch (SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (HasAttribute(SPELL_ATTR9_AFFECTED_BY_SPELLSWAP) || Id == 81209)
                if (Id != 15473 && // shadow form is not chakra spell
                    Id != 88625)   // Holy Word: Chastise is debuff and can stack with others
...
uint32 SpellInfo::GetMaxTicks() const
{
    ...
    for (uint8 x = 0; x < MAX_SPELL_EFFECTS; x++)
    {
        if (Effects[x].Effect == SPELL_EFFECT_APPLY_AURA ||
            Effects[x].Effect == SPELL_EFFECT_APPLY_AURA_2)
...
uint32 SpellInfo::CalcStackAmount(Unit const* caster) const
{
    uint32 stack = StackAmount;
    if (caster)
    {
        if (Player* modOwner = caster->GetSpellModOwner())
            modOwner->ApplySpellMod(Id, SPELLMOD_STACK_AMOUNT, stack);
    }
...
void SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, int32* m_powerCost) const
{
    ...
    for (auto itr : SpellPowers)
    {
        ...
        // Base powerCost
        int32 powerCost = itr->Cost;
        // PCT cost from total amount
        if (itr->CostBasePercentage)
        {
            switch (itr->powerType)
            {
                // health as power used
                case POWER_HEALTH:
                    powerCost += int32(CalculatePct(caster->GetCreateHealth(), itr->CostBasePercentage));
                    break;
                case POWER_MANA:
                ...
                case POWER_FOCUS:
                case POWER_ENERGY:
                    powerCost += int32(CalculatePct(caster->GetMaxPower(Powers(itr->powerType)), itr->CostBasePercentage));
                    break;
                case POWER_RUNES:
                case POWER_RUNIC_POWER:
                    ...
                    break;
                default:
                    sLog->outError(LOG_FILTER_SPELLS_AURAS, "CalculateManaCost: Unknown power type '%d' in spell %d", itr->powerType, Id);
        ...
        // Hack for Fel Flame
        if (itr->SpellId == 77799)
            powerCost = int32(powerCost * 2.0f);
        ...
        if (Player* modOwner = caster->GetSpellModOwner())
        {
            if (!modOwner->GetGlobalCooldownMgr().HasGlobalCooldown(this))
    ...
    {
        case 86213:
            m_powerCost[POWER_TO_INDEX(GetMainPower())] = 0;
            break;
        default:
            break;
    }
                ...
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
                    if (!deep)
                    {
                        if (SpellInfo const* spellTriggeredProto = sSpellMgr->GetSpellInfo(Effects[effIndex].TriggerSpell))
                        {
                            ...
                            for (uint8 i = 0; i < spellTriggeredProto->EffectCount; ++i)
                            {
...
bool SpellInfo::IsBreakingCamouflage() const
{
    ...
    // For trinkets
    if (Effects[0].ApplyAuraName == SPELL_AURA_MOD_STAT)
        return false;
    ...
    switch (Id)
    {
        ...
        case 86507: // Fortitude - from Runescroll of Fortitude II (item 62251)
            return false;
    }
...
bool SpellInfo::ShouldTriggerGiftOfTheSerpent() const
{
    switch (Id)
    {
        case 124041:
...
float SpellInfo::GetBonusValueForOldBonus(float defaultValue) const
{
    switch (Id)
    {
        ...
        case 126577:  // Inner Brilliance
            return 3653.0f;
        //case 132586:  //  PvP Bonus from 2H PVP Weapon, TODO: item stats must be depend on item level
            //return 7745.0f;
...
bool SpellInfo::IsBreakingStealth(Unit* m_caster) const
{
    ...
    if (m_caster && m_caster->HasAura(108208) && m_caster->HasAura(115191) && !m_caster->HasAura(115192) && !HasAttribute(SPELL_ATTR1_NOT_BREAK_STEALTH) && !m_caster->HasAura(51713))
    {
        ...
        if (Id == 32375 || Id == 32592 || Id == 72734 || Id == 39897 ||
            ...
            Id == 126734 ||  // Synapse Springs
            Id == 58838)     // Inherit Master's Threat List
            callSubterfuge = false;
...
{
    return !HasCustomAttribute(SpellCustomAttributes::SPELL_ATTR0_CU_TRIGGERED_IGNORE_RESILENCE);
}
