...
{
    std::vector<SpellLogEffectPowerDrainParams> _powerDrainTargets;
    std::vector<SpellLogEffectExtraAttacksParams> _extraAttacksTargets;
    std::vector<SpellLogEffectDurabilityDamageParams> _durabilityDamageTargets;
    std::vector<SpellLogEffectGenericVictimParams> _genericVictimTargets;
    std::vector<SpellLogEffectTradeSkillItemParams> _tradeSkillTargets;
    std::vector<SpellLogEffectFeedPetParams> _feedPetTargets;
};
...
class Spell
{
    ...
    public:
        ...
        void EffectPlaySceneObject(SpellEffIndex p_EffIndex);
        void EffectDeathGrip(SpellEffIndex p_EffIndex);
        ...
        bool cast(bool skipCheck = false);
        ...
        void SendSpellDispellLog(DispelChargesList const& list, bool isSteal, bool isBreak);
        ...
        void ExecuteLogEffectDurabilityDamage(uint8 effIndex, Unit* victim, int32 itemId, uint32 damage);
    ...
    protected:
        ...
        std::map<uint8, SpellLogEffect> m_effectExecuteData;
...
namespace JadeCore
{
    ...
    struct WorldObjectSpellConeTargetWithSrcPositionCheck : public WorldObjectSpellAreaTargetCheck
    {
        float _coneAngle;
        Position _srcPos;
        WorldObjectSpellConeTargetWithSrcPositionCheck(float coneAngle, float range, Unit* caster, Position &srcPos,
            SpellInfo const* spellInfo, SpellTargetCheckTypes selectionType, ConditionContainer* condList);
        bool operator()(WorldObject* target);
    };
