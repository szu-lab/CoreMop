        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch (m_spellInfo->Id)                     // better way to check unknown
                {
                    case 148008:
                    {
                        damage += int32(m_caster->SpellBaseDamageBonusDone(m_spellInfo->GetSchoolMask()) * (2.0f / 5.0f));
                        break;
                    }
            ...
            case SPELLFAMILY_WARRIOR:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    case 46968: // Shockwave
                    {
                        ...
                        switch (m_caster->ToPlayer()->GetSpecializationId())
                        {
                        ...
                        {
                            m_caster->NeedResetCooldownFromShockWave(true);
                            // hack way to prevent multiple calls of this condition
                            m_caster->ToPlayer()->AddSpellCooldown(136848, 0, 5 * IN_MILLISECONDS);
                        }
                    ...
                    case 2098:  // Eviscerate
                    {
                        if (m_caster->GetTypeId() == TYPEID_PLAYER)
                        {
                            if (uint32 combo = ((Player*)m_caster)->GetComboPoints())
                            {
                                ...
                                if (AuraEffect* eviscerateBonus = m_caster->GetAuraEffect(37169, EFFECT_0))
                    ...
                    case 26679: // Deadly Throw
                    {
                        if (m_caster->GetTypeId() == TYPEID_PLAYER)
                        {
                            if (uint32 combo = ((Player*)m_caster)->GetComboPoints())
                            {
                                ...
                                if (m_caster->ToPlayer()->GetSpecializationId() == SPEC_ROGUE_ASSASSINATION
                                    || m_caster->ToPlayer()->GetSpecializationId() == SPEC_ROGUE_COMBAT)
                                    damage += int32(ap * combo * 0.12f);
                                else if (m_caster->ToPlayer()->GetSpecializationId() == SPEC_ROGUE_SUBTLETY)
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            {
                case 133507:
                    m_caster->DealDamage(unitTarget, unitTarget->GetMaxHealth() + 1);
                    break;
                ...
                case 135712: // Grand Commendation of the Lorewalkers
                    ...
                    m_caster->ToPlayer()->GetSession()->AddReputationMod(m_spellInfo->Effects[effIndex].BasePoints, 100);
                    m_caster->ToPlayer()->GetReputationMgr().SendFactionBonusInfo();
                    break;
                ...
                case 6203:  // Soulstone
                {
                    if (!unitTarget->isAlive())
                        unitTarget->CastSpell(unitTarget, 3026, true, nullptr, nullptr, m_caster->GetGUID()); // Self resurrect
                    break;
                }
                ...
                case 136494:
                {
                    if (!unitTarget->IsFriendlyTo(m_caster))
                    {
                        if (m_caster->IsValidAttackTarget(unitTarget))
                            ...
                            m_caster->CastSpell(m_caster, 85673, true);
                    }
                    else
                        m_caster->CastSpell(unitTarget, 85673, true);
                    break;
                }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            switch (m_spellInfo->Id)
            {
                case 45529: // Blood Tap
                {
                    ...
                    if (runeIndex == 255)
                        return; // there is no depleted runes
    ...
    switch (m_spellInfo->Id)
    {
        case 142723:
        {
            if (!unitTarget || !unitTarget->IsInPartyWith(m_caster) || unitTarget->isDead())
                return;

            int32 casterHp = 0;
            if (m_caster->HealthBelowPct(25))
            {
                casterHp = 25;

                int32 value = m_caster->CountPctFromMaxHealth(25) - unitTarget->GetHealth(); // heal to 25%
                m_caster->CastCustomSpell(m_caster, 118594, nullptr/*damage*/, &value/*heal*/, nullptr, true);
            }
            else
                casterHp = m_caster->GetHealthPct();
            ...
            if (unitTarget->HealthBelowPct(25))
            {
                ...
                int32 value = unitTarget->CountPctFromMaxHealth(25) - unitTarget->GetHealth(); // heal to 25%
                unitTarget->CastCustomSpell(unitTarget, 118594, nullptr/*damage*/, &value/*heal*/, nullptr, true);
            }
            else
                targetHp = unitTarget->GetHealthPct();

            if (targetHp == casterHp)
                return;

            int diff = abs(casterHp - targetHp);
            if (casterHp > targetHp)
            {
                int value = m_caster->CountPctFromMaxHealth(diff);
                m_caster->ModifyHealth(-value);

                value = unitTarget->CountPctFromMaxHealth(diff);
                m_caster->CastCustomSpell(unitTarget, 118594, nullptr/*damage*/, &value/*heal*/, nullptr, true);
            }
            else
            {
                int value = unitTarget->CountPctFromMaxHealth(diff);
                unitTarget->ModifyHealth(-value);

                value = m_caster->CountPctFromMaxHealth(diff);
                m_caster->CastCustomSpell(m_caster, 118594, nullptr/*damage*/, &value/*heal*/, nullptr, true);
            }
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    else if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_TRIGGER_SPELL_WITH_VALUE
        && effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        /// Special Cases
        switch (triggered_spell_id)
        {
            /// Demonic Synergy (does not exist)
            case 104330:
            {
                if (Unit* l_Owner = m_caster->GetOwner())
                    l_Owner->EnergizeBySpell(l_Owner, m_spellInfo->Id, m_spellInfo->Effects[effIndex].BasePoints, Powers::POWER_DEMONIC_FURY);
    ...
    m_caster->CastSpell(targets, spellInfo, m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_TRIGGER_SPELL_WITH_VALUE ? &values : nullptr, TRIGGERED_FULL_MASK, NULL, nullptr, m_originalCasterGUID);
}
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    // original caster guid only for GO cast
    m_caster->CastSpell(targets, spellInfo, m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_TRIGGER_MISSILE_SPELL_WITH_VALUE ? &values : nullptr, TRIGGERED_FULL_MASK, NULL, nullptr, m_originalCasterGUID);
}
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    unitTarget->CastSpell(targets, spellInfo, m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_FORCE_CAST_WITH_VALUE ? &values : nullptr, TRIGGERED_FULL_MASK);
}
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
    ...
    float speedXY, speedZ, dist;
    dist = m_caster->GetExactDist2d(x, y);
    if (dist == 0.0f)
        return;
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 556:   // Astral Recall
            if (Player* target = unitTarget->ToPlayer())
            {
                ...
                if (target->HasAura(147787))
                {
                    if (target->GetTeamId() == TEAM_ALLIANCE)
                        m_targets.SetDst(-8833.07f, 622.78f, 95.03f, 6.27f, 0);
                    else
                        m_targets.SetDst(1569.97f, -4397.41f, 18.01f, 0.86f, 1);

                    AddDestTarget(*m_targets.GetDst(), 0);
                }
            }
            break;
    }
    ...
    {
        unitTarget->GetPosition(x, y, z, orientation);
        orientation += 180.f;
    ...
    // Fix Glyph of Rapid Teleportation - 46989
    if (unitTarget->getClass() == CLASS_MAGE && unitTarget->HasAura(89749))
        unitTarget->CastSpell(unitTarget, 46989, true);
...
void Spell::EffectApplyAura(SpellEffIndex effIndex)
{
    ...
    // Touch of Karma (absorb aura) - 122470, Anti-Magic Shell - 48707, Divine Aegis - 47753,  Mastery: Illuminated Healing - 76669, Spirit Shell - 114908 must ignore Battle Fatigue
    if (m_spellAura->GetId() != 122470 && m_spellAura->GetId() != 48707 && m_spellAura->GetId() != 47753 && m_spellAura->GetId() != 86273 && m_spellAura->GetId() != 114908)
    {
        AuraEffect* l_Effect = m_spellAura->GetEffect(effIndex);
        if (l_Effect && l_Effect->GetAuraType() == SPELL_AURA_SCHOOL_ABSORB)
        {
            float TakenTotalMod = 1.0f;
            if (auto amount = unitTarget->GetTotalAuraModifier(SPELL_AURA_MOD_ABSORPTION_PCT))
                AddPct(TakenTotalMod, amount);

            if (auto auraEffect = unitTarget->GetAuraEffect(110310, EFFECT_0)) ///< Dampening
                AddPct(TakenTotalMod, -auraEffect->GetAmount());

            int currentValue = (int)(l_Effect->GetAmount() * TakenTotalMod);

            l_Effect->SetAmount(currentValue);
        }
    }
...
void Spell::EffectHealPct(SpellEffIndex effIndex)
{
    ...
    heal = unitTarget->SpellHealingBonusTaken(m_originalCaster, m_spellInfo, (uint8)effIndex, heal, HEAL);
...
void Spell::EffectHealMechanical(SpellEffIndex effIndex)
{
    ...
    m_healing += unitTarget->SpellHealingBonusTaken(m_originalCaster, m_spellInfo, (uint8)effIndex, heal, HEAL);
}
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    if (m_caster->isAlive())
    {
        ...
        healthGain = m_caster->SpellHealingBonusTaken(m_caster, m_spellInfo, (uint8)effIndex, healthGain, HEAL);
...
void Spell::EffectCreateItem(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->IsAbilityOfSkillType(SKILL_ARCHAEOLOGY))
    {
        ...
        if (!found)
            m_caster->ToPlayer()->UpdateSkillPro(SKILL_ARCHAEOLOGY, 1000, 5);
    }
...
void Spell::EffectCreateItem2(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->Id)
    {
    case 113968: archId = 79896; break;
    case 113971: archId = 79897; break;
    case 113972: archId = 79898; break;
    case 113973: archId = 79899; break;
    case 113974: archId = 79900; break;
    case 113975: archId = 79901; break;
    case 113976: archId = 79902; break;
    case 113977: archId = 79903; break;
    case 113978: archId = 79904; break;
    case 113979: archId = 79905; break;
    case 113982: archId = 79908; break;
    case 113983: archId = 79909; break;
    case 113984: archId = 79910; break;
    case 113985: archId = 79911; break;
    case 113986: archId = 79912; break;
    case 113987: archId = 79913; break;
    case 113988: archId = 79914; break;
    case 113989: archId = 79915; break;
    case 113990: archId = 79916; break;
    case 113991: archId = 79917; break;
    case 139776: archId = 95375; break;
    case 139779: archId = 95376; break;
    case 139780: archId = 95377; break;
    case 139781: archId = 95378; break;
    case 139782: archId = 95379; break;
    case 139783: archId = 95380; break;
    case 139784: archId = 95381; break;
    case 139785: archId = 95382; break;
    }
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    ...
    if (!m_spellAura)
    {
        ...
        if (Aura* aura = Aura::TryCreate(m_spellInfo, MAX_EFFECT_MASK, dynObj, caster, &m_spellValue->EffectBasePoints[0], nullptr, 0, m_castItemLevel))
        {
            m_spellAura = aura;
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    // Mighty Rage Potion
    if (m_spellInfo->Id == 17528)
        damage = urand(450, 750);
    ...
    if (m_caster->HasAura(119975) && power == POWER_RUNIC_POWER) // Conversion
        return;
    ...
    if (gameObjTarget)
    {
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_BUTTON:
                ...
                player->GetMap()->ScriptsStart(sGameObjectScripts, gameObjTarget->GetSpawnId(), player, gameObjTarget);
                return;
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    if (effIndex == EFFECT_0 || effIndex == EFFECT_1)
    {
        switch (m_spellInfo->Id)
        {
            case 148597:
            ...
            case 130146:
            {
                ...
                m_caster->GetCreatureListWithEntryInGrid(summons, entry, 100.0f);
                for (Creature* summon : summons)
                {
                    if (summon->GetOwnerGUID() == m_caster->GetGUID())
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_UNK:
            if (((properties->Flags & 512) || m_spellInfo->Id == 114192 || m_spellInfo->Id == 114203 || m_spellInfo->Id == 114207) && m_spellInfo->Id != 3599)
            {
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_TOTEM:
                {
                    ...
                    // Stone Bulwark Totem
                    if (m_spellInfo->Id == 108270 || m_spellInfo->Id == 108280)
                    ...
                    if (AuraEffect* totemicVigorEffect = m_caster->GetAuraEffect(63298, EFFECT_0))
                    {
                        uint32 value = m_caster->CountPctFromMaxHealth(totemicVigorEffect->GetAmount()) + summon->GetMaxHealth();
                        summon->SetMaxHealth(value);
                        summon->SetHealth(value);
                    }
                    break;
                }
                case SUMMON_TYPE_MINIPET:
                {
                    summon = m_caster->GetMap()->SummonCreature(entry, *destTarget, properties, duration, m_originalCaster, m_spellInfo->Id, 0, 0LL, nullptr, false);
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    if (Aura* executionSentenceDamage = unitTarget->GetAura(114916))
    {
   ...
   // Check if player has Execution Sentence - 114917 heal aura
    else if (Aura* executionSentenceDamage = unitTarget->GetAura(114917))
    {
        executionCaster = executionSentenceDamage->GetCaster();
        if (executionCaster)
        {
            ...
            executionDamage = 12989 + spellPowerBonus;
            float PvPPower = 1.0f;
            if (Player* player = executionCaster->ToPlayer())
                PvPPower = player->GetPvpHealingBonus();
    ...
    /*make copy of dispel_list to handle cooldown of 527 and others*/
    unitTarget->GetDispellableAuraList(m_caster, dispelMask, m_dispellData);
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            if (AuraEffect* aurEff = m_caster->GetAuraEffect(119864, 0))
                m_caster->CastSpell(unitTarget, 119856, true);
        }
        // Remove Curse
        if (m_spellInfo->Id == 475)
        {
            if (AuraEffect* aurEff = m_caster->GetAuraEffect(115700, 0))
    ...
    SendSpellDispellLog(success_list, false, false);
...
void Spell::SendSpellDispellLog(DispelChargesList const& list, bool isSteal, bool isBreak)
{
    WorldPacket data(SMSG_SPELLDISPELLOG, 8 + 8 + 3 + 4 + list.size() * 5);
    ...
    {
        data.WriteBit(1); // unk bit 1
        data.WriteBit(0); // unk bit 2
        data.WriteBit(0); // unk bit 3
    }
    data.WriteBit(targetGuid[7]);
    data.WriteBit(casterGuid[7]);
    data.WriteBit(isBreak);
    data.WriteByteSeq(casterGuid[6]);
    data.WriteByteSeq(casterGuid[5]);
    data.WriteByteSeq(targetGuid[2]);
    data.WriteByteSeq(casterGuid[4]);
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/)
{
    ...
    {
        if (m_caster->HasAura(63268) && (unitTarget->GetCreatureType() == CREATURE_TYPE_HUMANOID) != 0) // Glyph of Disguise
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    // if with Soulborn we should add cooldown for 1 minute
    if (owner && owner->getClass() == CLASS_WARLOCK && owner->HasAura(74434))
    {
        if (owner->HasAura(108499))
        {
            owner->AddSpellCooldown(112868, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(112869, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(112867, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(112866, 0, 60 * IN_MILLISECONDS);
        }
        else
        {
            owner->AddSpellCooldown(712, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(691, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(697, 0, 60 * IN_MILLISECONDS);
            owner->AddSpellCooldown(688, 0, 60 * IN_MILLISECONDS);
        }
    ...
    // restore hp for warlock pet
    if (petentry && m_caster->getClass() == CLASS_WARLOCK)
        pet->SetFullHealth();
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/)
{
    ...
    {
        // Glyph of Tranquil Grip and a case when death grip is reflected
        if (m_caster->HasAura(63335) || m_caster->getClass() != CLASS_DEATH_KNIGHT)
            return;
    }
    ...
    for (uint32 j = effIndex + 1; j < m_spellInfo->EffectCount; ++j)
    {
        switch (m_spellInfo->Effects[j].Effect)
        {
            ...
            case SPELL_EFFECT_WEAPON_PERCENT_DAMAGE:
                // Fanatic Strike has two weapon effects for different targets
                if (m_spellInfo->Id != 110956)
            ...
            switch (m_spellInfo->Id)
            {
                case 30213: // Legion Strike
                {
                    if (Unit* owner = m_caster->GetOwner())
                    {
                        ...
                        if (owner->GetTypeId() == TYPEID_PLAYER)
                            if (owner->ToPlayer()->GetSpecializationId() == SPEC_WARLOCK_DEMONOLOGY)
...
void Spell::EffectHealMaxHealth(SpellEffIndex effIndex)
{
    ...
    m_healing += addhealth;
    m_healing = unitTarget->SpellHealingBonusTaken(m_caster, m_spellInfo, (uint8)effIndex, m_healing, HEAL);
}
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                case 104924:
                {
                    m_caster->CastSpell(unitTarget, 104287, true);
                    break;
                }
                ...
                case 129803: ///< Skymirror Image
                {
                    if (unitTarget)
                        unitTarget->CastSpell(m_caster, 127315, true); ///< Skymirror Image (Proc)
                    break;
                }
            }
            break;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags[1]&0x10000)
            {
                ...
                if (Unit* l_OriginalTarget = m_targets.GetUnitTarget())
                {
                    if (l_OriginalTarget == unitTarget)
                        break;

                    static std::array<uint32, 2> l_Plagues = { 55078, 55095 }; ///< Blood Plague and Frost Fever

                    bool l_VisualApplied = false;
                    for (uint32 l_SpellId : l_Plagues)
                    {
                        if (l_OriginalTarget->HasAura(l_SpellId, m_caster->GetGUID()))
                        {
                            m_caster->CastSpell(unitTarget, l_SpellId, true);

                            SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(l_SpellId);
                            if (!l_SpellInfo)
                                continue;

                            for (uint8 l_EffectIndex = 0; l_EffectIndex < l_SpellInfo->EffectCount; ++l_EffectIndex)
                            {
                                if (auto newEffect = unitTarget->GetAuraEffect(l_SpellId, l_EffectIndex))
                                {
                                    if (auto oldEffect = l_OriginalTarget->GetAuraEffect(l_SpellId, l_EffectIndex))
                                    {
                                        std::memcpy(&newEffect->m_fixed_periodic, &oldEffect->m_fixed_periodic, sizeof(AuraEffect::FixedPeriodic));
                            ...
                            if (!l_VisualApplied)
                            {
                                m_caster->AddAura(63687, m_caster); // Cosmetic - Pestilence State
                                l_OriginalTarget->CastSpell(unitTarget, 91939, true); // Cosmetic - Send Diseases on target
                                l_VisualApplied = true;
                            }
                        }
                    }
                }
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    // Send request
    WorldPacket data(SMSG_DUEL_REQUESTED, 1 + 1 + 8 + 8);
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
    ...
    if (uint32 glyph = m_spellInfo->Effects[effIndex].MiscValue)
    {
        if (GlyphPropertiesEntry const* gp = sGlyphPropertiesStore.LookupEntry(glyph))
        {
            ...
            result = player->HandleGlyph(glyph, m_glyphIndex, true);
        }
    }
    else
        result = player->HandleGlyph(0, m_glyphIndex, false);
...
void Spell::EffectInebriate(SpellEffIndex /*effIndex*/)
{
    ...
    Player* player = unitTarget->ToPlayer();
    int32 currentDrunk = player->GetDrunkValue();
    currentDrunk = std::max(currentDrunk + damage, 0);
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
    ...
    Map* map = m_caster->GetMap();
    if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(), go_id, map,
...
void Spell::EffectResurrect(SpellEffIndex effIndex)
{
    ...
    uint32 healthPct = damage;
    uint32 manaPct = damage;

    // Special values coming from others effects
    switch (m_spellInfo->Id)
    {
        case 3026: ///< Soulstone
        case 126393: ///< Eternal Guardian
            healthPct = m_spellInfo->Effects[EFFECT_1].BasePoints;
            break;
        case 20484: ///< Rebirth
        {
            healthPct = m_spellInfo->Effects[EFFECT_1].BasePoints;
            ...
            /// Glyph of Rebirth
            if (Unit* l_OriginalCaster = GetOriginalCaster() ? GetOriginalCaster() : m_caster)
            {
                if (AuraEffect* l_RebirthGlyph = l_OriginalCaster->GetAuraEffect(54733, EFFECT_1))
                    healthPct = l_RebirthGlyph->GetAmount();
            }
            break;
        }
        case 61999: ///< Raise Ally
            manaPct = m_spellInfo->Effects[EFFECT_1].BasePoints;
            break;
        case 113269: ///< Rebirth (Symbiosis)
            healthPct = 60; ///< FIXME: Why 60% instead of 20%?
            break;
    }
    ...
    uint32 mana = CalculatePct(target->GetMaxPower(POWER_MANA), manaPct);
...
void Spell::EffectReputation(SpellEffIndex effIndex)
{
    ...
    if (int32 modifier = player->GetSession()->GetReputationModifier(faction_id))
        bonus += float(CalculatePct(rep_change, modifier));
...
void Spell::EffectSelfResurrect(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        {
            damage = m_spellInfo->Effects[EFFECT_1].BasePoints;

            auto _caster = GetOriginalCaster() ? GetOriginalCaster() : m_caster;
            if (_caster)
                if (_caster->HasAura(56231))
                    damage += 40;
        }
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
    ...
    unitTarget->KnockbackFrom(x, y, speedxy, speedz, m_spellInfo->Id == 126408);
...
void Spell::EffectLeapBack(SpellEffIndex effIndex)
{
    ...
    float speedxy = m_spellInfo->Effects[effIndex].MiscValue * 0.1f;
    float speedz = damage * 0.1f;
    bool back = true;
    ...
    // Disengage - 781
    if (m_spellInfo->Id == 781 && m_caster->GetTypeId() == TYPEID_PLAYER)
        m_caster->ToPlayer()->SetFallInformation(0, m_caster->GetPositionZ());
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    else if (goinfo->type == GAMEOBJECT_TYPE_SUMMONING_RITUAL || m_spellInfo->Id == 127249)
    {
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex)
{
    ...
    uint32 m_tickNumberOld = 0;
    bool stolenExecution = false;

    for (DispelChargesList::iterator itr = success_list.begin(); itr != success_list.end(); ++itr)
    {
        Aura* _aura = itr->first;

        // Stay of Execution - we should save tick numbers
        if (_aura->GetId() == 114917)
        {
            if (Aura* testAura = unitTarget->GetAura(_aura->GetId(), unitTarget->GetGUID()))
            {
                stolenExecution = true;
                m_tickNumberOld = testAura->GetEffect(0)->GetTickNumber();
            }
        }

        unitTarget->RemoveAurasDueToSpellBySteal(_aura->GetId(), _aura->GetCasterGUID(), m_caster);
    }

    SendSpellDispellLog(success_list, true, false);

    // Stay of Execution (114917) - restore tick numbers
    if (stolenExecution)
    {
        if (Aura* testAura = m_caster->GetAura(114917))
            testAura->GetEffect(0)->SetTickNumber(m_tickNumberOld);
    }
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        {
            summon->setFaction(caster->getFaction());
            summon->SetByteValue(UNIT_FIELD_BYTES_2, 1, caster->GetByteValue(UNIT_FIELD_BYTES_2, 1));
        }
...
void Spell::EffectUnlearnTalent(SpellEffIndex effIndex)
{
    ...
    for (auto itr : *plr->GetTalentMap(plr->GetActiveSpec()))
    {
        ...
        for (uint8 i = 0; i < spell->EffectCount; ++i)
...
void Spell::EffectCreateAreatrigger(SpellEffIndex effIndex)
{
    ...
    // Lightning Whip, Lei Shen, Throne of Thunder
    if (m_spellInfo->Id == 137499)
    {
        if (Creature* pCreature = m_caster->ToCreature())
        {
            float o = pCreature->AI()->GetFloatData(0);
            l_Source.SetOrientation(o);
        }
    }
    ...
    auto _caster = m_originalCaster && m_caster != m_originalCaster ? m_originalCaster : m_caster;
    AreaTrigger* areaTrigger = new AreaTrigger;
    if (!areaTrigger->CreateAreaTriggerFromSpell(_caster->GetMap()->GenerateLowGuid<HighGuid::AreaTrigger>(), _caster, GetSpellInfo(), effIndex, l_Source, l_Dest))
    {
        delete areaTrigger;
        return;
    }
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 119031: // Mastery : Gift of the Serpent
        {
            if (m_caster->HasAura(120482)) // Glyph of Enduring Healing Sphere
                areaTrigger->SetDuration(areaTrigger->GetDuration() + 3 * MINUTE * IN_MILLISECONDS);
            break;
        }
        ...
        case 147494: // Healing Sphere (Glyph of Fortuitous Sphere)
        {
            ...
            m_caster->GetAreaTriggerList(healingSphereList, 115460);
            m_caster->GetAreaTriggerList(healingSphereList, 147494);
...
void Spell::EffectPlaySceneObject(SpellEffIndex p_EffIndex)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT)
        return;
    ...
    uint32 sceneId = m_spellInfo->Effects[p_EffIndex].MiscValue;
...
void Spell::EffectStampede(SpellEffIndex p_EffIndex)
{
    ...
    bool   l_OnlyCurrentPet = l_Player->HasAuraType(AuraType::SPELL_AURA_STAMPEDE_ONLY_CURRENT_PET);

    uint32 l_CurrentSlot = l_Player->m_currentPetSlot;
    for (uint32 l_PetSlotIndex = uint32(PetSlot::PET_SLOT_HUNTER_FIRST); l_PetSlotIndex <= uint32(PetSlot::PET_SLOT_HUNTER_LAST); ++l_PetSlotIndex)
    {
        if (l_PetSlotIndex != l_CurrentSlot)
        {
            float l_X, l_Y, l_Z;
            l_Player->GetClosePoint(l_X, l_Y, l_Z, l_Player->GetObjectSize());
            Pet* l_Pet = l_Player->SummonPet(0, l_X, l_Y, l_Z, l_Player->GetOrientation(), PetType::SUMMON_PET, l_Player->CalcSpellDuration(GetSpellInfo()),
                PetSlot(l_OnlyCurrentPet ? l_CurrentSlot : l_PetSlotIndex), true);
            if (!l_Pet)
                return;

            if (!l_Pet->isAlive())
                l_Pet->setDeathState(DeathState::ALIVE);

            /// Set pet at full health
            l_Pet->SetHealth(l_Pet->GetMaxHealth());
            l_Pet->SetReactState(ReactStates::REACT_HELPER);
            l_Pet->m_Stampeded = true;

            /// Summoned pets with Stamped don't use abilities
            l_Pet->m_autospells.clear();
            l_Pet->m_spells.clear();
            l_Pet->m_Events.KillAllEvents(true);    ///< Disable automatic cast spells

            l_Pet->RemoveAllAuras();
            l_Pet->SetUInt32Value(EUnitFields::UNIT_CREATED_BY_SPELL, GetSpellInfo()->Id);
