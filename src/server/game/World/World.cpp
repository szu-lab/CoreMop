...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_int_configs[CONFIG_COMPRESSION_SIZE] = ConfigMgr::GetIntDefault("CompressionSize", 0);
    ...
    m_int_configs[CONFIG_SERVER_DELAY_SAVE_TIME] = ConfigMgr::GetIntDefault("ServerDelaySaveInterval", 300000);
    ...
    m_bool_configs[CONFIG_ANTICHEAT_GAMEMASTER_REPORT] = ConfigMgr::GetBoolDefault("Anticheat.GamemasterReport", false);
    m_int_configs[CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION] = ConfigMgr::GetIntDefault("Anticheat.ReportsForIngameWarnings", 70);
    m_int_configs[CONFIG_ANTICHEAT_TELEPORTHACKDETECTION_DISTANCE] = ConfigMgr::GetIntDefault("Anticheat.TeleportHackDetection.Distance", 30);
    m_int_configs[CONFIG_ANTICHEAT_TELEPORTHACKDETECTION_REPORTS_INGAME_NOTIFICATION] = ConfigMgr::GetIntDefault("Anticheat.TeleportHackDetection.ReportsForIngameWarnings", 0);
    ...
    m_bool_configs[CONFIG_ANTICHEAT_AUTOBAN_ENABLE] = ConfigMgr::GetBoolDefault("Anticheat.Autoban.Enable", false);
    m_int_configs[CONFIG_ANTICHEAT_AUTOBAN_TOTAL_REPORTS] = ConfigMgr::GetIntDefault("Anticheat.Autoban.TotalReports", 3000);
    m_int_configs[CONFIG_ANTICHEAT_AUTOBAN_TELEPORT_HACK_TOTAL_REPORTS] = ConfigMgr::GetIntDefault("Anticheat.Autoban.TeleportHack.TotalReports", 100);
    m_float_configs[CONFIG_ANTICHEAT_AUTOBAN_AVERAGE_REPORTS] = ConfigMgr::GetFloatDefault("Anticheat.Autoban.AverageReports", 0.5f);
    m_int_configs[CONFIG_ANTICHEAT_AUTOBAN_MAX_PING] = ConfigMgr::GetIntDefault("Anticheat.Autoban.MaxPing", 300);
    m_int_configs[CONFIG_ANTICHEAT_AUTOBAN_DURATION] = ConfigMgr::GetIntDefault("Anticheat.Autoban.Duration", 2592000);
    ...
    m_int_configs[CONFIG_PET_BATTLE_LEVEL_DIFF] = ConfigMgr::GetIntDefault("LFB.PetLevelDiff", 5);

    m_int_configs[CONFIG_RAID_ENCOUNTERS_RESET_DAYS] = ConfigMgr::GetIntDefault("RaidEncounters.ResetDays", 7);

    // Custom Transmogrify
    m_bool_configs[CONFIG_TRANSMOGRIFY_LEGENDARY_ENABLED] = ConfigMgr::GetBoolDefault("Transmogrify.Legendary.Enable", 1);
    m_int_configs[CONFIG_TRANSMOGRIFY_LEGENDARY_COST] = ConfigMgr::GetIntDefault("Transmogrify.Legendary.Cost", 20000) * GOLD;

    // Custom selected player rates
    m_int_configs[CONFIG_SKILL_CHANCE_ORANGE_LIMIT] = ConfigMgr::GetIntDefault("Custom.SkillChance.Orange.Limit", 100);
    m_int_configs[CONFIG_SKILL_CHANCE_YELLOW_LIMIT] = ConfigMgr::GetIntDefault("Custom.SkillChance.Yellow.Limit", 75);
    m_int_configs[CONFIG_SKILL_CHANCE_GREEN_LIMIT]  = ConfigMgr::GetIntDefault("Custom.SkillChance.Green.Limit", 25);
    m_int_configs[CONFIG_SKILL_CHANCE_GREY_LIMIT]   = ConfigMgr::GetIntDefault("Custom.SkillChance.Grey.Limit", 0);

    m_int_configs[CONFIG_SKILL_GAIN_CRAFTING_LIMIT]  = ConfigMgr::GetIntDefault("Custom.SkillGain.Crafting.Limit", 1);

    m_int_configs[CONFIG_SKILL_GAIN_GATHERING_LIMIT]  = ConfigMgr::GetIntDefault("Custom.SkillGain.Gathering.Limit", 1);

    rate_values[RATE_REPUTATION_GAIN_LIMIT]  = ConfigMgr::GetFloatDefault("Custom.Rate.Reputation.Gain.Limit", 1.0f);
    rate_values[RATE_REPUTATION_LOWLEVEL_KILL_LIMIT]  = ConfigMgr::GetFloatDefault("Custom.Rate.Reputation.LowLevel.Kill.Limit", 1.0f);
    rate_values[RATE_REPUTATION_LOWLEVEL_QUEST_LIMIT]  = ConfigMgr::GetFloatDefault("Custom.Rate.Reputation.LowLevel.Quest.Limit", 1.0f);

    rate_values[RATE_XP_KILL_LIMIT]       = ConfigMgr::GetFloatDefault("Custom.Rate.XP.Kill.Limit", 1.0f);
    rate_values[RATE_XP_QUEST_LIMIT]      = ConfigMgr::GetFloatDefault("Custom.Rate.XP.Quest.Limit", 1.0f);
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading SpellAreaTriggers names...");
    sObjectMgr->LoadSpellAreaTriggers();
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Raid Encounters...");
    sObjectMgr->LoadRaidEncounterData();

    sRaidEncountersMgr->LoadFromDB();

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Consumable Item Spells...");
    sSpellMgr->LoadSpellConsumableItems();
    ...
    sLog->outInfo(LOG_FILTER_GENERAL, "Loading item upgrade hotfix...");
    sObjectMgr->LoadItemUpgradeHotfix();
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        {
            LoginDatabase.PExecute("UPDATE realmlist set online=%u, queued=%u where id=%u", GetActiveSessionCount(), uint32(m_QueuedPlayer.size()), realmID);
    ...
    if (m_serverDelayTimer >= m_int_configs[CONFIG_SERVER_DELAY_SAVE_TIME])
    {
        ...
        LoginDatabase.PExecute("UPDATE realmlist set delay=%u where id=%u", delay, realmID);
        m_serverDelayTimer -= m_int_configs[CONFIG_SERVER_DELAY_SAVE_TIME];
    }
    else
...
void World::SendGlobalGMMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second &&
            itr->second->GetPlayer() &&
            itr->second->GetPlayer()->IsInWorld() &&
            !itr->second->GetPlayer()->isSpectator() &&
            itr->second != self &&
            AccountMgr::IsGMAccount(itr->second->GetSecurity()) &&
            (team == 0 || itr->second->GetPlayer()->GetTeam() == team))
        {
            itr->second->SendPacket(packet);
        }
...
void World::SendGMText(int32 string_id, ...)
{
    ...
    for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if (itr->second->GetPlayer()->isSpectator())
            continue;

        if (!AccountMgr::IsGMAccount(itr->second->GetSecurity()))
            continue;
...
void World::SendGMText(const char* text)
{
    WorldPacket packet;
    ChatHandler::FillMessageData(&packet, NULL, CHAT_MSG_SYSTEM, LANG_UNIVERSAL, NULL, 0, text, NULL);
...
{
    Player* pBanned = sObjectAccessor->FindPlayerByNameInOrOutOfWorld(name.c_str());
    uint32 guid = 0;
...
void World::InitRaidEncountersResetTime()
{
    ...
    int resetDays = getIntConfig(CONFIG_RAID_ENCOUNTERS_RESET_DAYS);
    ...
    if (!resetTime)
    {
        resetTime = mktime(&localTm) + (resetDays * DAY);
    ...
    else
    {
        ...
        {
            sRaidEncountersMgr->ResetEncounters();
...
void World::ResetCurrencyWeekCap()
{
    ...
    sLog->OutTrinity("World::ResetCurrencyWeekCap()");
}
...
void World::LoadCharacterInfoStore()
{
    ...
    {
        Field* fields = result->Fetch();
        AddCharacterInfo(fields[0].GetUInt32(), fields[2].GetUInt32(), fields[1].GetString(),
...
void World::AddCharacterInfo(uint32 guid, uint32 accountId, std::string const& name, uint8 gender, uint8 race, uint8 playerClass, uint8 level)
{
