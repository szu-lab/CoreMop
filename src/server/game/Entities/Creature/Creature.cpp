...
void Creature::RemoveCorpse(bool setSpawnTime)
{
    ...
    // Sometimes falling creatures get stuck with falling spline. Create new spline before we relocate
    delete movespline;
...
bool Creature::UpdateEntry(uint32 Entry, uint32 team, const CreatureData* data)
{
    ...
    if (GetCreatureTemplate()->InhabitType & INHABIT_AIR && isInAir && !HasUnitMovementFlag(MOVEMENTFLAG_FALLING))
    {
    ...
    switch (m_deathState)
    {
        ...
        case DEAD:
        {
            ...
            if (m_respawnTime <= now)
            {
                ...
                uint64 dbtableHighGuid = MAKE_NEW_GUID(m_spawnId, GetEntry(), HighGuid::Unit);
...
void Creature::SelectLevel(const CreatureTemplate* cinfo)
{
    ...
    SetBaseWeaponDamage(RANGED_ATTACK, MINDAMAGE, (bg_mindmg ? bg_mindmg : cinfo->minrangedmg * damagemod));
    SetBaseWeaponDamage(RANGED_ATTACK, MAXDAMAGE, (bg_maxdmg ? bg_maxdmg : cinfo->maxrangedmg * damagemod));
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId, uint32 team, const CreatureData* data)
{
    ...
    Object::_Create(guidlow, Entry, vehId ? HighGuid::Vehicle : HighGuid::Unit);
...
bool Creature::LoadCreatureFromDB(uint32 spawnId, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->outError(LOG_FILTER_SQL, "Creature (GUID: %u) not found in table `creature`, can't load. ", spawnId);
        return false;
    }

    m_spawnId = spawnId;
    if (!Create(map->GenerateLowGuid<HighGuid::Unit>(), map, data->phaseMask, data->id, 0, 0, data->posX, data->posY, data->posZ, data->orientation, data))
        return false;
...
void Creature::SetFixedAttackDistance(const float distance)
{
    ASSERT(distance >= 0.0f);

    m_FixedAttackDistance = distance;
}

float Creature::GetFixedAttackDistance() const
{
    return m_FixedAttackDistance;
}
...
void Creature::setDeathState(DeathState s)
{
    ...
    else if (s == JUST_RESPAWNED)
    {
        ...
        LoadCreaturesAddon(true); ///< Must be done after setDeathState(ALIVE)
    }
}
...
void Creature::DespawnCreaturesInArea(uint32 entry, float radius /*= 100.f*/)
{
    std::vector<uint32> entryList;
    entryList.push_back(entry);
    DespawnCreaturesInArea(entryList, radius);
}

void Creature::DespawnCreaturesInArea(std::vector<uint32> entryList, float radius /*=100.f*/)
{
    std::list<Creature*> creatures;
    for (std::vector<uint32>::iterator itr = entryList.begin(); itr != entryList.end(); ++itr)
        GetCreatureListWithEntryInGrid(creatures, *itr, radius);
...
bool Creature::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index, bool permanent) const
{
    ...
    return Unit::IsImmunedToSpellEffect(spellInfo, index, permanent);
}
...
bool Creature::canCreatureAttack(Unit const* victim, bool /*force*/) const
{
    ...
    dist = std::max(dist, GetFixedAttackDistance());
...
void Creature::ProhibitSpellSchool(SpellSchoolMask idSchoolMask, uint32 unTimeMs)
{
    ...
    for (uint8 i = 0; i < CREATURE_MAX_SPELLS; ++i)
    {
        ...
        if ((spellInfo->PreventionType & SPELL_PREVENTION_TYPE_SILENCE) == 0)
            continue;
...
bool Creature::SetHover(bool enable)
{
    ...
    if (enable)
        PacketSender(this, SMSG_SPLINE_MOVE_SET_HOVER, SMSG_MOVE_SET_HOVER, false).Send();
    else
        PacketSender(this, SMSG_SPLINE_MOVE_UNSET_HOVER, SMSG_MOVE_UNSET_HOVER, false).Send();
