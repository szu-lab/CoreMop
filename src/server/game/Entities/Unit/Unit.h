...
enum SpellAuraInterruptFlags
{
    ...
    AURA_INTERRUPT_FLAG_LOGOUT                      = 0x80000000,   ///< 31
...
enum TriggerCastFlags
{
    ...
    TRIGGERED_IGNORE_LINE_OF_SIGHT_CHECK            = 0x00080000,   //! Will ignore LOS and vision checks
    TRIGGERED_FULL_MASK                             = 0xFFFFFFFF & ~TRIGGERED_IGNORE_LINE_OF_SIGHT_CHECK,
};
...
enum UnitState
{
    ...
    UNIT_STATE_IGNORE_PATHFINDING = 0x20000000,
...
private:
    ...
    HealInfo* _healInfo;
public:
    explicit ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell const* spell, DamageInfo* damageInfo, HealInfo* healInfo);
...
enum Stagger
{
    STAGGER_DAMAGE_EFFECT      = 124255,
    LIGHT_STAGGER_EFFECT       = 124275,
    MODERATE_STAGGER_EFFECT    = 124274,
    HEAVY_STAGGER_EFFECT       = 124273,
};
...
class Unit : public WorldObject
{
    public:
        ...
        Unit* SelectNearbyTarget(Unit* exclude = NULL, float dist = NOMINAL_MELEE_RANGE, bool checkValidAttack = false, SpellInfo const* spellInfo = nullptr) const;
        Unit* SelectNearbyAlly(Unit* exclude = NULL, float dist = NOMINAL_MELEE_RANGE, bool checkValidAssist = false, SpellInfo const* spellInfo = nullptr) const;
        ...
        void Mount(uint32 mount, uint32 vehicleId = 0, uint32 creatureEntry = 0, bool isTaxi = false);
        void Dismount();
        ...
        void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb = 0, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, Aura const* procEffect = nullptr, Spell const* spell = nullptr);
        void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb = 0, SpellInfo const* procAura = NULL, Aura const* procEffect = nullptr, Spell const* spell = nullptr);
        ...
        bool CastSpell(SpellCastTargets const& p_Targets, SpellInfo const* p_SpellInfo, CustomSpellValues const* p_Values, TriggerCastFlags p_TriggerFlags = TRIGGERED_NONE, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0, float p_PeriodicDamageModifier = 0.0f);
        bool CastSpell(Unit* p_Target, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0, float p_PeriodicDamageModifier = 0.0f);
        bool CastSpell(Unit* p_Target, uint32 p_SpellId, TriggerCastFlags p_TriggerFlags = TRIGGERED_NONE, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0, float p_PeriodicDamageModifier = 0.0f);
        bool CastSpell(Unit* p_Target, SpellInfo const* p_SpellInfo, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastSpell(Unit* p_Target, SpellInfo const* p_SpellInfo, TriggerCastFlags p_TriggerFlags, Item* p_CastItem, AuraEffect const* p_TriggeredByAura, uint64 p_OriginalCasterGUID, float p_PeriodicDamageModifier = 0.0f);
        bool CastCustomSpell(Unit* p_Target, uint32 p_SpellId, int32 const* p_BP0, int32 const* p_BP1, int32 const* p_BP2, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastCustomSpell(Unit* p_Target, uint32 p_SpellId, int32 const* p_BP0, int32 const* p_BP1, int32 const* p_BP2, int32 const* p_BP3, int32 const* p_BP4, int32 const* p_BP5, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastCustomSpell(uint32 p_SpellId, SpellValueMod p_ValueMod, int32 p_Value, Unit* p_Target = nullptr, bool p_Triggered = true, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastCustomSpell(uint32 p_SpellId, CustomSpellValues const& p_Values, Unit* p_Target = nullptr, bool p_Triggered = true, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastSpell(Position const& p_Pos, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastSpell(float p_X, float p_Y, float p_Z, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect const* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        bool CastSpell(GameObject* p_Object, uint32 p_SpellId, bool p_Triggered, Item* p_CastItem = nullptr, AuraEffect* p_TriggeredByAura = nullptr, uint64 p_OriginalCasterGUID = 0);
        ...
        void NearTeleportTo(Position p_Pos, bool p_Casting = false);
        void SendTeleportPacket(Position &newPos);
        ...
        void KnockbackFrom(float x, float y, float speedXY, float speedZ, bool useObjectRotation = false);
        ...
        */
        ...
        void RegisterAuraApplyTime(Aura* aura);
        void UnRegisterAuraApplyTime(Aura* aura);

        bool IsLatestAppliedAuras(Aura* aura) const;
        ...
        int32 GetTotalAuraModifier(AuraType p_AuraType, std::function<bool(AuraEffect* /*p_AuraEffect*/, int32& /*p_Amount*/)> const& p_AuraFunc) const;
        int32 GetTotalAuraModifierByEquippedItemClass(AuraType auratype, int32 itemClass) const;
        ...
        int32 GetTotalAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask) const;
        float GetTotalAuraMultiplierByMiscBMask(AuraType auratype, uint32 misc_mask) const;
        int32 GetMaxPositiveAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask, AuraEffect const* except = nullptr) const;
        int32 GetMaxNegativeAuraModifierByMiscBMask(AuraType auratype, uint32 misc_mask) const;
        ...
        uint32 SpellHealingBonusTaken(Unit* caster, SpellInfo const *spellProto, uint8 effectIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack = 1);
        ...
        virtual bool IsImmunedToSpell(SpellInfo const* spellInfo, bool permanent = false);
        ...
        virtual bool IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index, bool permanent = false) const;
                                                            // redefined in Creature
        ...
        SpellSchoolMask CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo = NULL);
        ...
        void AddHealDamageLog(uint32 value, HealDamageLogType type);
        ...
        // borrowed time handler
        void NeedToRemoveBorrowedTime(bool remove) { removeBorrowTime = remove; }
        bool IsNeedToRemoveBorrowedTime() { return removeBorrowTime; }

        // for real spent power cost on spell
        void SetSpentPowerCost(int32 cost) { SpentPowerCost = cost; }
        int32 GetSpentPowerCost() { return SpentPowerCost; }
        ...
        template<typename F>
        void ForEachAreaTrigger(F p_Func)
        {
            ForEach(m_AreaTrigger, std::move(p_Func));
        }

        template<typename F>
        void ForEachGameObject(F p_Func)
        {
            ForEach(m_gameObj, std::move(p_Func));
        }
    ...
    protected:
        ...
        public:
        std::unordered_map<uint32, std::list<time_t>> m_aurasCastTime;
        ...
        typedef std::vector<HealDamageLog> HealDamageLogList;
    ...
    private:
        ...
        bool HandleAuraProcOnPowerAmount(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const *procSpell, uint32 procFlag, uint32 procEx);
        bool HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 absorb, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, Aura const* procEffect);
        ...
        bool HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorb, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, bool& applyCooldown);
    ...
    class HealthPctOrderPred
    {
        public:
            ...
            bool operator() (const WorldObject* a, const WorldObject* b) const
            {
                ...
                return this->operator()(a->ToUnit(), b->ToUnit());
            }
        private:
            const bool m_ascending;
    };
