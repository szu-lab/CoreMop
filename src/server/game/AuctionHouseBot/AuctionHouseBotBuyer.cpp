...
bool AuctionBotBuyer::Initialize()
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot buyer interval is %u minutes", _checkInterval / MINUTE);
    return true;
}
...
bool AuctionBotBuyer::Update(AuctionHouseType houseType)
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: %s buying ...", AuctionBotConfig::GetHouseTypeName(houseType));
...
uint32 AuctionBotBuyer::GetItemInformation(BuyerConfiguration& config)
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: %u items added to buyable/biddable vector for ah type: %u", count, config.GetHouseType());
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: SameItemInfo size = %u", (uint32)config.SameItemInfo.size());
    return count;
}
...
bool AuctionBotBuyer::RollBuyChance(const BuyerItemInfo* ahInfo, const Item* item, const AuctionEntry* auction, uint32 /*bidPrice*/)
{
    ...
    if (ahInfo)
    {
        ...
        //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: buyout average: %.1f items with buyout: %u", avgBuyPrice, ahInfo->BuyItemCount);
    ...
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: %s BUY! chance = %.2f, price = %u, buyprice = %u.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBuyPrice));
    return win;
}
...
bool AuctionBotBuyer::RollBidChance(const BuyerItemInfo* ahInfo, const Item* item, const AuctionEntry* auction, uint32 bidPrice)
{
    ...
    if (ahInfo)
    {
        ...
        //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Bid average: %.1f biddable item count: %u", avgBidPrice, ahInfo->BidItemCount);
    ...
    float rand = frand(0.f, 100.f);
    bool win = rand <= chance;
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: %s BID! chance = %.2f, price = %u, bidprice = %u.", win ? "WIN" : "LOSE", chance, uint32(itemPrice), uint32(itemBidPrice));
    return win;
}
...
void AuctionBotBuyer::PrepareListOfEntry(BuyerConfiguration& config)
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: EligibleItems size = %u", (uint32)config.EligibleItems.size());
}
...
void AuctionBotBuyer::BuyAndBidItems(BuyerConfiguration& config)
{
    ...
    if (items.size() > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Boost value used for Buyer! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    ...
    while (cycles && itr != items.end())
    {
        ...
        if (!auction)
        {
            //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Entry %u doesn't exists, perhaps bought already?", itr->second.AuctionId);
            items.erase(itr++);
            continue;
        }
        ...
        {
            //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: In time interval wait for entry %u!", auction->Id);
            ++itr;
            continue;
        }
        ...
        //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Rolling for AHentry %u:", auction->Id);
...
{
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Entry %u bought at %.2fg", auction->Id, float(auction->buyout) / GOLD);
...
{
    //TC_LOG_DEBUG(LOG_FILTER_AHBOT, "AHBot: Bid placed to entry %u, %.2fg", auction->Id, float(bidPrice) / GOLD);
