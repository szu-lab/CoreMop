...
bool AuctionBotSeller::Initialize()
{
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot seller filters:");
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Forced Inclusion %u items", (uint32)includeItems.size());
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Forced Exclusion %u items", (uint32)excludeItems.size());

    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Loading npc vendor items for filter..");
    ...
    for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
    {
        ...
        if (const VendorItemData* data = sObjectMgr->GetNpcVendorItemList(l_Entry))
        {
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Npc vendor filter has %u items", (uint32)npcItems.size());

    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Loading loot items for filter..");
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Loot filter has %u items", (uint32)lootItems.size());
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Sorting and cleaning items for AHBot seller...");
    ...
    if (!itemsAdded)
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "AuctionHouseBot seller not have items, disabled.");
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AuctionHouseBot seller will use %u items to fill auction house (according your config choices)", itemsAdded);

    LoadConfig();

    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Items loaded \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
        sLog->outDebug(LOG_FILTER_WORLDSERVER, "\t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot seller configuration data loaded and initialized");
    return true;
}
...
void AuctionBotSeller::LoadSellerValues(SellerConfiguration& config)
{
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: minTime = %u", config.GetMinTime());
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: maxTime = %u", config.GetMaxTime());

    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: For AH type %u", config.GetHouseType());
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: GrayItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GRAY));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: WhiteItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_WHITE));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: GreenItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GREEN));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: BlueItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_BLUE));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: PurpleItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_PURPLE));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: OrangeItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_ORANGE));
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: YellowItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_YELLOW));
}
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration& config)
{
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: Missed Item       \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    {
        sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: \t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration& config)
{
    ...
    if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: Boost value used to fill AH! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    else
    ...
    while (GetItemsToSell(config, itemsToSell, allItems) && items > 0)
    {
        ...
        if (!itemId)
        {
            sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: Item entry 0 auction creating attempt.");
            continue;
        }
        ...
        if (!prototype)
        {
            sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: Unknown item %u auction creating attempt.", itemId);
            continue;
        }
        ...
        if (!item)
        {
            sLog->outError(LOG_FILTER_WORLDSERVER, "AHBot: Item::CreateItem() returned NULL for item %u (stack: %u)", itemId, stackCount);
            return;
        }
        ...
        auctionEntry->itemGUIDLow = item->GetGUID();
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: Added %u items to auction", count);
}
...
bool AuctionBotSeller::Update(AuctionHouseType houseType)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_WORLDSERVER, "AHBot: %s selling ...", AuctionBotConfig::GetHouseTypeName(houseType));
