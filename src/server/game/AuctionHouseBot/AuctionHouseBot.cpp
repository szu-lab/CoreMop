...
bool AuctionBotConfig::Initialize()
{
    ...
    {
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "AHBOT is Disabled.");
        return false;
    }
    ...
    {
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "All feature of AuctionHouseBot are disabled!");
        return false;
    }

    if (GetConfig(CONFIG_AHBOT_ALLIANCE_ITEM_AMOUNT_RATIO) == 0 && GetConfig(CONFIG_AHBOT_HORDE_ITEM_AMOUNT_RATIO) == 0 && GetConfig(CONFIG_AHBOT_NEUTRAL_ITEM_AMOUNT_RATIO) == 0)
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "AuctionHouseBot SELLER is disabled!");
    ...
    if (!GetConfig(CONFIG_AHBOT_BUYER_ALLIANCE_ENABLED) && !GetConfig(CONFIG_AHBOT_BUYER_HORDE_ENABLED) && !GetConfig(CONFIG_AHBOT_BUYER_NEUTRAL_ENABLED))
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "AuctionHouseBot BUYER is disabled!");
...
void AuctionBotConfig::SetConfig(AuctionBotConfigUInt32Values index, char const* fieldname, uint32 defvalue)
{
    ...
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "AHBot: %s (%i) can't be negative. Using %u instead.", fieldname, int32(GetConfig(index)), defvalue);
...
void AuctionBotConfig::SetConfigMax(AuctionBotConfigUInt32Values index, char const* fieldname, uint32 defvalue, uint32 maxvalue)
{
    ...
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "AHBot: %s (%u) must be in range 0...%u. Using %u instead.", fieldname, GetConfig(index), maxvalue, maxvalue);
...
void AuctionBotConfig::SetConfigMinMax(AuctionBotConfigUInt32Values index, char const* fieldname, uint32 defvalue, uint32 minvalue, uint32 maxvalue)
{
    ...
    if (GetConfig(index) > maxvalue)
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "AHBot: %s (%u) must be in range %u...%u. Using %u instead.", fieldname, GetConfig(index), minvalue, maxvalue, maxvalue);
        SetConfig(index, maxvalue);
    }

    if (GetConfig(index) < minvalue)
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "AHBot: %s (%u) must be in range %u...%u. Using %u instead.", fieldname, GetConfig(index), minvalue, maxvalue, minvalue);
...
{
    SetConfig(index, ConfigMgr::GetFloatDefault(fieldname, defvalue));
}
