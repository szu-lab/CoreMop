...
std::map<uint32, CriteriaEntry const*>     sCriteriaMap;
...
std::map<uint32, std::vector<ModifierTreeEntry const*>> sModifierTreeMap;
std::map<uint32, std::vector<CriteriaTreeEntry const*>> sCriteriaTreeMap;
std::map<uint32, std::vector<CriteriaTreeEntry const*>> sLinkedCriteriaTreeMap;
std::map<uint32 /*criteriaTree*/, AchievementEntry const*> sCriteriaAchievementMap;
std::map<uint32 /*old criteriaId*/, uint32 /*criteriaTreeId*/> sOldCriteriaToNewMap;
...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sCriteriaTreeStore,           dbcPath, "CriteriaTree.dbc");
    ...
    {
        if (CriteriaEntry const* criteria = sCriteriaStore.LookupEntry(i))
        {
            sCriteriaMap[criteria->ID] = criteria;
        }
    }

    for (uint32 j = 0; j < sModifierTreeStore.GetNumRows(); ++j)
    {
        if (ModifierTreeEntry const* tree = sModifierTreeStore.LookupEntry(j))
        {
            sModifierTreeMap[tree->Parent].push_back(tree);
        }
    }

    for (uint32 j = 0; j < sCriteriaTreeStore.GetNumRows(); ++j)
    {
        if (CriteriaTreeEntry const* tree = sCriteriaTreeStore.LookupEntry(j))
        {
            sCriteriaTreeMap[tree->Parent].push_back(tree);
        }
    }
    ...
    for (uint32 j = 0; j < sCriteriaTreeStore.GetNumRows(); ++j)
    {
        if (CriteriaTreeEntry const* tree = sCriteriaTreeStore.LookupEntry(j))
        {
            ...
            auto subData = sCriteriaTreeMap[tree->ID];
            if (subData.size() != 2)
                continue;
            ...
            for (CriteriaTreeEntry const* temp : subData)
            {
                const_cast<CriteriaTreeEntry*>(temp)->Flags |= 1 << (10 + temp->OrderIndex);
            }
    ...
    {
        AchievementEntry* achievement = sAchievementStore.LookupEntry(i);
        if (!achievement)
            continue;
        ...
        {
        case 252:
            const_cast<AchievementEntry*>(achievement)->flags |= ACHIEVEMENT_FLAG_SUMM;
            break;
        }
        ...
        FillCriteriaTree(achievement->CriteriaTree, achievement, sLinkedCriteriaTreeMap[achievement->CriteriaTree]);
    }

    auto result = WorldDatabase.PQuery("SELECT criteriaId, treeId, type FROM cold_cnew ORDER BY criteriaId DESC");
    if (result)
    {
        do
        {
            ...
            uint32 oldCriteriaId = fields[0].GetUInt32();
            uint32 newCriteriaTreeId = fields[1].GetUInt32();
            uint8 type = fields[2].GetUInt8();

            if (oldCriteriaId == 0 || newCriteriaTreeId == 0)
                continue;

            if (!sCriteriaTreeStore.LookupEntry(newCriteriaTreeId))
            {
                sLog->outError(LOG_FILTER_SERVER_LOADING, "cold_cnew table contains non-existed CriteriaTreeId %u for AchievementCriteriaId (old) %u. Skip!", newCriteriaTreeId, oldCriteriaId);
                continue;
            }

            sOldCriteriaToNewMap[oldCriteriaId] = newCriteriaTreeId;
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sgtItemSocketCostPerLevelStore, dbcPath, "gtItemSocketCostPerLevel.dbc");                                   // 17399
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sPlayerConditionStore,        dbcPath, "PlayerCondition.dbc");                                              // 17399
    ...
    for (uint32 i = 0; i < sLFGDungeonStore.GetNumRows(); ++i)
    {
        ...
        if (lfdEntry->difficulty != SCENARIO_DIFFICULTY && lfdEntry->difficulty != SCENARIO_HEROIC_DIFFICULTY)
            continue;
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellScalingStore,             dbcPath,"SpellScaling.dbc");                                                  // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellTotemsStore,              dbcPath,"SpellTotems.dbc");                                                   // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellTargetRestrictionsStore,  dbcPath,"SpellTargetRestrictions.dbc");                                       // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellPowerStore,               dbcPath,"SpellPower.dbc");                                                    // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellLevelsStore,              dbcPath,"SpellLevels.dbc");                                                   // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellInterruptsStore,          dbcPath,"SpellInterrupts.dbc");                                               // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellEquippedItemsStore,       dbcPath,"SpellEquippedItems.dbc");                                            // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellClassOptionsStore,        dbcPath,"SpellClassOptions.dbc");                                             // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellCooldownsStore,           dbcPath,"SpellCooldowns.dbc");                                                // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellAuraOptionsStore,         dbcPath,"SpellAuraOptions.dbc");                                              // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellProcsPerMinuteStore,      dbcPath,"SpellProcsPerMinute.dbc");                                           // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellProcsPerMinuteModStore,   dbcPath,"SpellProcsPerMinuteMod.dbc");                                        // 18414
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellAuraRestrictionsStore,    dbcPath,"SpellAuraRestrictions.dbc");                                         // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellCastingRequirementsStore, dbcPath,"SpellCastingRequirements.dbc");                                      // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellCategoriesStore,          dbcPath,"SpellCategories.dbc");                                               // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellCategoryStores,           dbcPath,"SpellCategory.dbc");                                                 // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellEffectStore,              dbcPath,"SpellEffect.dbc");                                                   // 18414
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellEffectScalingStore,       dbcPath,"SpellEffectScaling.dbc");                                            // 18414
    ...
    {
        if (SummonPropertiesEntry* prop = sSummonPropertiesStore.LookupEntry(i))
        {
...
{
    const static std::string l_NullName = "";
...
{
    if (inventoryType <= INVTYPE_ROBE && (armorType < 4 || armorType == ITEM_SUBCLASS_ARMOR_SHIELD-1))
    {
        switch (inventoryType)
        {
            case INVTYPE_NON_EQUIP:
...
void FillCriteriaTree(uint32 treeId, AchievementEntry const* achievement, std::vector<CriteriaTreeEntry const*>& list)
{
    if (!treeId || !achievement)
        return;

    auto const& itr = sCriteriaTreeMap.find(treeId);
    if (itr == sCriteriaTreeMap.end())
        return;

    for (CriteriaTreeEntry const* tree : itr->second)
    {
        if (tree->CriteriaID == 0)
            FillCriteriaTree(tree->ID, achievement, list);
        else
        {
            list.push_back(tree);

            sCriteriaAchievementMap[tree->ID] = achievement;
        }
    }
}

uint32 GetCriteriaTreeFromOldCriteria(uint32 oldCriteriaId)
{
    if (oldCriteriaId == 0)
        return 0;

    auto const& itr = sOldCriteriaToNewMap.find(oldCriteriaId);
    if (itr == sOldCriteriaToNewMap.end())
        return 0;
