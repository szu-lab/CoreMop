    ...
    m_OutActive(false), m_Seed(static_cast<uint32> (rand32())), m_zstream()
{
    ...
    int z_res = deflateInit2(m_zstream, sWorld->getIntConfig(CONFIG_COMPRESSION), Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY);
    if (z_res != Z_OK)
    {
...
int WorldSocket::SendPacket(WorldPacket const* pct)
{
    ...
    //    if (m_Session && m_Session->GetAccountId() == 205641)
    ...
    size_t size2 = pct->size();
    uint32 opcode = pct->GetOpcode();

    int compressionSize = sWorld->getIntConfig(CONFIG_COMPRESSION_SIZE);
    bool compressable = compressionSize > 0 &&
        size >= (size_t)compressionSize
        && opcode != MSG_VERIFY_CONNECTIVITY
        && opcode != SMSG_MOTD
        && opcode != SMSG_MESSAGE_CHAT
        ...
        && opcode != SMSG_MOVE_UPDATE 
        && opcode != SMSG_MONSTER_MOVE;

    if (compressable)
    {
        uint32 bufferSize = deflateBound(m_zstream, pct->size() + sizeof(opcode));

        size_t reserved_size = bufferSize + 12;
        compressed.resize(reserved_size);
        compressed.put<uint32>(0, size + 4);
        
        uint32 adler_1 = adler32(adler32(0x9827D8F1, (Bytef*)&opcode, 4), pct->contents(), size);
        ...
        m_zstream->next_in = (Bytef*)&opcode;
        m_zstream->avail_in = 4;

        int z_res = deflate(m_zstream, Z_BLOCK);
        if (z_res != Z_OK)
        {
            //sLog->outError(LOG_FILTER_NETWORKIO, "Can't compress packet opcode (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), m_zstream->msg);
            return 0;
        }

        m_zstream->next_in = (Bytef*)pct->contents();
        m_zstream->avail_in = pct->size();

        z_res = deflate(m_zstream, Z_SYNC_FLUSH);
        if (z_res != Z_OK)
        {
            //sLog->outError(LOG_FILTER_NETWORKIO, "Can't compress packet data (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), m_zstream->msg);
            return 0;
        }

        uint32 compressedSize = bufferSize - m_zstream->avail_out;;
        uint32 adler_2 = (uint32)adler32(2552748273u, (const Bytef*)(compressed.contents() + 12), compressedSize);
        ...
        {
            compressed.resize(compressedSize + 12);
    ...
    /*if (false && compressable)
    {
        auto const& str = GetOpcodeNameForLogging((Opcodes)opcode, WOW_SERVER);
        sLog->outInfo(LOG_FILTER_OPCODES, "Compress packet %s from %u to %u (%f)", str.c_str(), size2, size, 100.0f - ((((float)size) / (float)size2) * 100.0f));
    }*/
...
int WorldSocket::handle_input_header (void)
{
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        ACE_NEW_RETURN(m_RecvWPct, WorldPacket(opcodeNumber, header.size), -1);
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    /// Query the opcode name only if required (costly call)
    std::string opcodeName;
    auto l_GetOpcodeName = [&opcodeName, opcode]() -> char const*
    {
        if (opcodeName.empty())
            opcodeName = GetOpcodeNameForLogging(opcode, WOW_CLIENT);

        return opcodeName.c_str();
    };

    if (opcode != CMSG_PLAYER_MOVE && sLog->ShouldLog(LOG_FILTER_OPCODES, LOG_LEVEL_INFO))
        sLog->outInfo(LOG_FILTER_OPCODES, "C->S: %s", l_GetOpcodeName());
        ...
        switch (opcode)
        {
            ...
            case CMSG_KEEP_ALIVE:
            {
                if (sLog->ShouldLog(LOG_FILTER_NETWORKIO, LOG_LEVEL_DEBUG))
                    sLog->outDebug(LOG_FILTER_OPCODES, l_GetOpcodeName());
