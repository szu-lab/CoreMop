...
{
    WorldPacket data(SMSG_GUILD_COMMAND_RESULT, 9);
    data << uint32(errCode);
    data << uint32(type);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    WorldPacket data(SMSG_GUILD_BANK_QUERY_TEXT_RESULT, 4 + 2 + size);

    data.WriteBits(size, 14);
...
m_achievementMgr(this), _level(1), _experience(0), _todayExperience(0), _newsLog(this), m_NeedToBeSaved(false), m_saveTimer(0), m_saveDelay(0) 
{
    int interval = sWorld->getIntConfig(CONFIG_GUILD_SAVE_INTERVAL) * MINUTE * IN_MILLISECONDS;

    m_saveDelay = interval + irand(-interval/10, interval/10);

    memset(&m_bankEventLog, 0, (GUILD_BANK_MAX_TABS + 1) * sizeof(LogHolder*));

    m_ChallengeCount.resize(GuildChallengeType::ChallengeMax);

    for (uint8 l_Type = 0; l_Type < GuildChallengeType::ChallengeMax; ++l_Type)
...
bool Guild::Create(Player* pLeader, const std::string& name)
{
    ...
    for (uint8 l_Itr = 1; l_Itr < GuildChallengeType::ChallengeMax; ++l_Itr)
    {
...
void Guild::Update(int const diff)
{
    if (m_saveTimer >= m_saveDelay)
    {
        SaveToDB();
        m_saveTimer = 0;
    }
...
{
    auto mesize = m_members.size();
    auto mosize = m_motd.length();
    auto misize = m_info.length();

    WorldPacket data(SMSG_GUILD_ROSTER, 16 + 5 + mesize + mosize + misize + (mesize*4) + (mesize*125));
    ByteBuffer memberData(mesize*125);
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        if (!player)
            player = ObjectAccessor::FindPlayerInOrOutOfWorld(member->GetGUID());

        bool inInterRealm = false;
        if (player && player->GetSession()->GetInterRealmBG())
            inInterRealm = true;
        ...
        uint32 zoneId = member->GetZone();
        if (inInterRealm)
            zoneId = player->GetSession()->GetInterRealmBG();
        else if (player)
            zoneId = player->GetZoneId();
        memberData << uint32(zoneId);
        memberData << uint8(1);
...
void Guild::HandleInviteMember(WorldSession* session, const std::string& name)
{
    ...
    if (!pInvitee->GetSocial() || pInvitee->GetSocial()->HasIgnore(player->GetGUIDLow()))
        return;
    ...
    ObjectGuid oldGuildGuid = MAKE_NEW_GUID(pInvitee->GetGuildId(), 0, pInvitee->GetGuildId() ? HighGuid::Guild : (HighGuid)0);
...
void Guild::HandleSetMemberRank(WorldSession* session, uint64 targetGuid, uint64 setterGuid, uint32 rank)
{
    ...
    if (Member* member = GetMember(targetGuid))
    {
        ...
        Member* setter = GetMember(setterGuid);
        if (!setter)
        {
            SendCommandResult(session, GUILD_INVITE_S, ERR_GUILD_INTERNAL);
            return;
        }

        if (setter->GetRankId() != 1 && setter->GetRankId() >= member->GetRankId())
        {
            SendCommandResult(session, GUILD_INVITE_S, ERR_GUILD_PERMISSIONS);
            return;
        }
...
void Guild::HandleMemberLogout(WorldSession* session)
{
    ...
    // Login event
    WorldPacket data(SMSG_GUILD_SEND_PLAYER_LOGIN_STATUS, 2 + 8 + 8 + 4);
    data.WriteBit(playerGuid[0]);
    data.WriteBit(0);
    data.WriteBit(playerGuid[1]);
    data.WriteBit(playerGuid[4]);
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    WorldPacket data(SMSG_GUILD_PERMISSIONS_QUERY_RESULTS, 12 + 3 + 64);
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    */
    ...
    // Login event
    data.Initialize(SMSG_GUILD_SEND_PLAYER_LOGIN_STATUS, 1 + 1 + 8 + 8 + 4);
    data.WriteBit(playerGuid[0]);
    data.WriteBit(0);
    data.WriteBit(playerGuid[1]);
    data.WriteBit(playerGuid[4]);
...
void Guild::UpdatePerksFor(Player* player)
{
    ASSERT(player != NULL);
    ASSERT(player->GetGuildId() == GetId());

    for (uint32 i = 0; i < sGuildPerkSpellsStore.GetNumRows(); ++i)
    {
        GuildPerkSpellsEntry const* entry = sGuildPerkSpellsStore.LookupEntry(i);
        if (!entry)
            continue;
...
void Guild::RemovePerksFor(Player* player)
{
    ASSERT(player != NULL);
...
{
    WorldPacket data(SMSG_GUILD_RECIPES, 2);
...
bool Guild::LoadFromDB(Field* fields)
{
    ...
    if (m_bankMoney > MAX_MONEY_AMOUNT)
        m_bankMoney = MAX_MONEY_AMOUNT;
...
bool Guild::LoadGuildChallengesFromDB(Field* p_Fields)
{
    ...
    if (l_ChallengeType >= GuildChallengeType::ChallengeMax)
        return false;
        ...
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        {
            if (Player* player = itr->second->FindPlayer())
            {
                if (player->GetSession() && _HasRankRight(player, officerOnly ? GR_RIGHT_OFFCHATLISTEN : GR_RIGHT_GCHATLISTEN) &&
                    (player->GetSocial() && !player->GetSocial()->HasIgnore(session->GetPlayer()->GetGUIDLow())))
                {
...
{
    if (p_ChallengeType >= GuildChallengeType::ChallengeMax)
        return;
...
void Guild::SendGuildChallengeUpdated(WorldSession* p_Session /*= nullptr*/)
{
    ...
    for (uint8 l_I = 0; l_I < GuildChallengeType::ChallengeMax; l_I++)
