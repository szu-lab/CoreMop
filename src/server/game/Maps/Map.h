...
class Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        void SetVisibilityRange(float p_NewRange) { m_VisibleDistance = p_NewRange; }
        ...
        bool Is25ManRaid() const { return IsRaid() && (i_spawnMode == MAN25_DIFFICULTY || i_spawnMode == MAN25_HEROIC_DIFFICULTY || i_spawnMode == RAID_TOOL_DIFFICULTY); }   // since 25man difficulties are 1 and 3, we can check them like that
        ...
        TempSummon* SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties = NULL, uint32 duration = 0, Unit* summoner = NULL, uint32 spellId = 0, uint32 vehId = 0, uint64 viewerGuid = 0, std::list<uint64>* viewersList = NULL, bool count = true);
        ...
        AreaTrigger* GetAreaTrigger(uint64 guid);
        ...
        Corpse* GetCorpseByPlayer(uint64 ownerGuid) const
        {
        ...
        /*
            RESPAWN TIMES
        */
        time_t GetLinkedRespawnTime(uint64 guid);
        time_t GetCreatureRespawnTime(uint32 dbGuid);
        time_t GetGORespawnTime(uint32 dbGuid);
        ...
        Corpse* ConvertCorpseToBones(uint64 ownerGuid, bool insignia = false);
        ...
        void SetRaidEncounter(RaidEncounter* encounter) { m_RaidEncounter = encounter; }
...
class InstanceMap : public Map
{
    public:
        ...
        void CreateInstanceData(RaidEncounter* encounter);
        void LoadInstanceDataFromEncounter(RaidEncounter* p_Encounter);
        bool Reset(uint8 method);
        uint32 GetScriptId() { return i_script_id; }
        InstanceScript* GetInstanceScript() { return i_data; }
        void PermBindAllPlayers(Player* source);
        void PermBindAllPlayersForRaid(Player* source);
        void UnloadAll();
