...
void Map::LoadMMap(int gx, int gy)
{
    ...
    if (mmapLoadResult)
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "MMAP loaded name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
    else
        sLog->outInfo(LOG_FILTER_WORLDSERVER, "Could not load MMAP name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
}
...
_creatureToMoveLock(false), _gameObjectsToMoveLock(false), _dynamicObjectsToMoveLock(false), _unloading(false),
...
bool Map::AddPlayerToMap(Player* player)
{
    ...
    if (player && player->IsInWorld() &&
        player->isSpectator() && player->isSpectateRemoving())
...
bool Map::AddToMap(Transport* obj)
{
    ...
    if (!GetPlayers().isEmpty())
    {
        for (Map::PlayerList::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            ...
            {
                UpdateData data(itr->getSource()->GetMapId());
...
uint16 Map::GetAreaFlag(float x, float y, float z, bool *isOutdoors) const
{
    ...
    else
    {
        ...
        // HACK: for some maps, because areaflag cannot be properly calculated from getArea
        if (i_mapEntry->MapID == 1130)
        {
            areaflag = GetAreaFlagByMapId(i_mapEntry->MapID);
        }
    }
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
...

    {
        ...
        // Dungeon only code
        //if (IsRaid() /*temporary only for ToT*/ && GetId() == 1098)
        if (IsRaid())
        {
            RaidEncounter* encounter = GetRaidEncounter();
            if (!encounter)
                return false;

            RaidEncounter* playerEncounter = sRaidEncountersMgr->AddEncounterForPlayer(player->GetGUIDLow(), encounter->GetMapId(), encounter->GetEncounterMask(), encounter->data);

            if (encounter != playerEncounter)
            {
                if ((encounter->GetEncounterMask() | playerEncounter->GetEncounterMask()) != encounter->GetEncounterMask())
                {
                    return false;
                }

                if (encounter->IsBinded())
                {
                    playerEncounter->m_mapid = encounter->m_mapid;
                    playerEncounter->m_encounterMask = encounter->m_encounterMask;
                    playerEncounter->data = encounter->data;
                    playerEncounter->SaveToDB();
                    playerEncounter->SwapRespawnTimesFromEncounter(encounter);
...
void InstanceMap::CreateInstanceData(RaidEncounter* encounter)
{
    if (i_data != NULL)
        return;
    ...
    if (encounter != NULL)
    {
        i_data->SetCompletedEncountersMask(encounter->m_encounterMask);
        if (!encounter->data.empty())
        {
            sLog->outDebug(LOG_FILTER_MAPS, "Loading instance data for `%s` with id %u", sObjectMgr->GetScriptName(i_script_id), i_InstanceId);
            i_data->Load(encounter->data.c_str());
        }
    }
}

void InstanceMap::LoadInstanceDataFromEncounter(RaidEncounter* p_Encounter)
{
    if (i_data == nullptr)
        return;

    if (p_Encounter != nullptr)
    {
        i_data->SetCompletedEncountersMask(p_Encounter->m_encounterMask);

        if (!p_Encounter->data.empty())
        {
            sLog->outDebug(LOG_FILTER_MAPS, "Loading instance data for `%s` with id %u", sObjectMgr->GetScriptName(i_script_id), i_InstanceId);
            i_data->Load(p_Encounter->data.c_str());
        }
    }
    else
        i_data->Initialize();
...
void InstanceMap::PermBindAllPlayersForRaid(Player* source)
{
    if (!IsDungeon())
        return;
    ...
    uint32 playerGuidLow = group ? GUID_LOPART(group->GetLeaderGUID()) : source->GetGUIDLow();
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        RaidEncounter* playerEncounter = sRaidEncountersMgr->AddEncounterForPlayer(player->GetGUIDLow(), encounter->m_mapid, encounter->m_encounterMask, encounter->data);
        playerEncounter->SaveToDB();
...
AreaTrigger* Map::GetAreaTrigger(uint64 guid)
{
...
{
    if (GUID_HIPART(guid) != HighGuid::Mo_Transport)
        return NULL;
...
void Map::SaveCreatureRespawnTime(uint32 dbGuid, time_t respawnTime)
{
    ...
    if (RaidEncounter* encounter = GetRaidEncounter())
    {
        encounter->SaveCreatureRespawnTime(dbGuid, respawnTime);
    }
    else
    {
...
void Map::RemoveCreatureRespawnTime(uint32 dbGuid)
{
    ...
    if (RaidEncounter* encounter = GetRaidEncounter())
    { 
        encounter->RemoveCreatureRespawnTime(dbGuid);
    }
    else
    {
...
void Map::SaveGORespawnTime(uint32 dbGuid, time_t respawnTime)
{
    ...
    if (RaidEncounter* encounter = GetRaidEncounter())
    {
        encounter->SaveGameObjectRespawnTime(dbGuid, uint32(respawnTime));
    }
    else
    {
...
void Map::RemoveGORespawnTime(uint32 dbGuid)
{
    ...
    if (RaidEncounter* encounter = GetRaidEncounter())
    {
        encounter->RemoveGameObjectRespawnTime(dbGuid);
    }
    else
    {
...
time_t Map::GetCreatureRespawnTime(uint32 dbGuid)
{
    if (RaidEncounter* encounter = GetRaidEncounter())
    {
        return encounter->GetCreatureRespawnTime(dbGuid);
    }
    else
    {
...
time_t Map::GetGORespawnTime(uint32 dbGuid)
{
    if (RaidEncounter* encounter = GetRaidEncounter())
    {
        return encounter->GetGameObjectRespawnTime(dbGuid);
    }
    else
    {
...
time_t Map::GetLinkedRespawnTime(uint64 guid)
{
...
void Map::LoadCorpseData()
{
    ...
    // SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask, guid FROM corpse WHERE mapId = ? AND instanceId = ?
    PreparedQueryResult result = CharacterDatabase.Query(stmt);
    if (!result)
        return;
...
Corpse* Map::ConvertCorpseToBones(uint64 ownerGuid, bool insignia /*= false*/)
{
