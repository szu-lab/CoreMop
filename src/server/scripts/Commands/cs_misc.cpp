...
public:
    ...
    static bool HandleAppearCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            ...
            else if (map->IsDungeon())
            {
                ...
                {
                    if (RaidEncounter* targetEncounter = sRaidEncountersMgr->GetEncounterForPlayerAndMap(target->GetGUIDLow(), target->GetMapId()))
                    {
                        sRaidEncountersMgr->AddEncounterForPlayer(_player->GetGUIDLow(), targetEncounter->GetMapId(), targetEncounter->GetEncounterMask(), targetEncounter->data);
                        _player->SetGMRaidTarget(target->GetGUID());
                    }
                }
                else
                {
    ...
    static bool HandleKickPlayerCommand(ChatHandler* handler, char const* args)
    {
        ...
        target = sObjectAccessor->FindPlayerByNameInOrOutOfWorld(name.c_str());
        if (!target)
            return false;
