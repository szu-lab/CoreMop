...
public:
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        _owner->GetMotionMaster()->MovePoint(EVENT_CHARGE, *_owner);

        Movement::MoveSplineInit init(*_owner);
        init.DisableTransportPathTransformations();
        init.MoveTo(_dest.GetPositionX(), _dest.GetPositionY(), _dest.GetPositionZ());
        init.Launch();
...
struct gunship_npc_AI : public ScriptedAI
{
    ...
    void SetData(uint32 type, uint32 data) override
    {
        if (type == ACTION_SET_SLOT && data < MAX_SLOTS)
        {
            ...
            me->GetMotionMaster()->MovePoint(EVENT_CHARGE, Slot->TargetPosition);
...
public:
    ...
    struct npc_gunshipAI : public NullCreatureAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (isVictory)
            {
                if (Transport* otherTransport = HashMapHolder<Transport>::Find(instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE)))
    ...
    struct npc_high_overlord_saurfang_igbAI : public ScriptedAI
    {
        ...
        void DoAction(const int32 action) override
        {
            ...
            else if (action == ACTION_EXIT_SHIP)
            {
                ...
                me->GetMotionMaster()->MovePoint(EVENT_CHARGE_PREPATH, pos);
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_ADDS:
                    ...
                    if (Transport* skybreaker = HashMapHolder<Transport>::Find(_instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE)))
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_ADDS:
                    ...
                    if (Transport* orgrimsHammer = HashMapHolder<Transport>::Find(_instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE)))
...
struct npc_gunship_boarding_addAI : public gunship_npc_AI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        if (pointId == EVENT_CHARGE && Slot)
        {
            ...
            if (Transport* destTransport = HashMapHolder<Transport>::Find(Instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE)))
        ...
        bool CheckAreaTarget(Unit* target)
        {
            if (Player* l_Player = GetOwner()->ToPlayer())
            {
                if (InstanceScript* l_Instance = l_Player->GetInstanceScript())
                    return l_Instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) != DONE;
            }
...
public:
    ...
    class spell_igb_burning_pitch_SpellScript : public SpellScript
    {
        ...
        {
            PreventHitDefaultEffect(effIndex);
            GetCaster()->CastCustomSpell(uint32(GetEffectValue()), SPELLVALUE_BASE_POINT0, 8000, NULL, true);
...
public:
    ...
    class spell_igb_rocket_artillery_explosion_SpellScript : public SpellScript
    {
        ...
        {
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                GetCaster()->CastCustomSpell(instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE ? SPELL_BURNING_PITCH_DAMAGE_A : SPELL_BURNING_PITCH_DAMAGE_H, SPELLVALUE_BASE_POINT0, 5000, NULL, true);
        }
    ...
    class spell_igb_gunship_fall_teleport_SpellScript : public SpellScript
    {
        ...
        void SelectTransport(WorldObject*& target)
        {
            if (InstanceScript* instance = target->GetInstanceScript())
                target = HashMapHolder<Transport>::Find(instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE));
        }
