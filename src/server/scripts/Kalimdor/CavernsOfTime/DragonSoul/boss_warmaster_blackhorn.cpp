    ...
    public:
        ...
        struct boss_warmaster_blackhornAI : public BossAI
        {
            ...
            void DoAction(const int32 action)
            {
                ...
                {
                    summons.DespawnAll();
                    DespawnCreaturesInArea(NPC_TWILIGHT_FLAMES, me);
                    events.Reset();
                    if (Creature* pShip = me->FindNearestCreature(NPC_SKYFIRE, 300.0f))
                    {
                    ...
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYFIRE_HARPOON_GUN, 200.0f); // TODO: adjuste searche range
                    if (creatures.size() == 2)
                        for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                            (*itr)->AI()->DoAction(ACTION_END_BATTLE);

                    creatures.clear();
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYFIRE_CANNON, 200.0f); // TODO: adjuste searche range
                    if (!creatures.empty())
                        for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                            {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            std::list<Creature*> creatures;
                            me->GetCreatureListWithEntryInGrid(creatures, NPC_ONSLAUGHT_TARGET, 300.0f); // TODO: adjuste searche range
                            if (!creatures.empty())
...
class npc_warmaster_blackhorn_skyfire_harpoon_gun: public CreatureScript
{
    public:
        ...
        private:
            ...
            {
                std::list<Creature*> creatures;
                me->GetCreatureListWithEntryInGrid(creatures, entry, 200.0f); // TODO: adjuste searche range
                if (!creatures.empty())
                    for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
