        ...
        struct npc_chogall_darkened_creationAI : public Scripted_NoMovementAI
        {
            ...
            void SpellHit(Unit* caster, SpellInfo const* spell)
            {
                for (uint8 i = 0; i < 3; ++i)
                {
                    ...
                    if (auto currentSpell = me->GetCurrentSpell(CURRENT_GENERIC_SPELL))
                    {
                        uint32 id = currentSpell->m_spellInfo->Id;
                        if (id == SPELL_DEBILITATING_BEAM || id == SPELL_DEBILITATING_BEAM_25 || id == SPELL_DEBILITATING_BEAM_10H || id == SPELL_DEBILITATING_BEAM_25H)
                        {
